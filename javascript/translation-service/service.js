...
export class TranslationService {
   ...
   */
  async request(text, count = 3) {
    // NOTE: explicit call 3 times "unrolled"
    /*
    const requestText = (text) =>
      new Promise((resolve, reject) =>
        this.api.request(text, (err) => (err ? reject(err) : resolve()))
      );
    return requestText(text).catch(() =>
      requestText(text).catch(() => requestText(text))
    );
     */
    ...
    return this.api
      ...
      .catch((error) => {
        if (error instanceof NotAvailable) {
          ...
          // NOTE: unrolled version below, call to premium above guarantees minimumQuality
          // .then((result) => {
          //   if (result.quality < minimumQuality) {
          //     throw new QualityThresholdNotMet(result.translation);
          //   }
